...
function autosizeTextLikeInput(el, wrapper){
  // универсал для "псевдо-input" на базе textarea и обычных input
  const cs = getComputedStyle(el);
  const box = parseFloat(cs.paddingLeft) + parseFloat(cs.paddingRight)
            + parseFloat(cs.borderLeftWidth) + parseFloat(cs.borderRightWidth);
  const extra = 28;
  const value = el.value || el.placeholder || '';
  const need = measureTextWidth(el, value) + box + extra;
  fitWidthInColumn(el, wrapper, need);
  // высота — чтобы не было скролла
...
function bindAutosize(el, wrapper, kind){
  ...
  if (field.type === 'text'){
    ...
    input.style.overflowWrap = 'anywhere';   // перенос длинных слов
